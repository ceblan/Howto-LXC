...
We split the CLUSTER-1 set up in three stages, each one with its own ansible
playbook:

1. [[id:214611e0-4085-451e-bfb1-fba2f6a2152e][CLUSTER-1 lxc playbook]]
2. [[id:6a9266c3-768a-4d72-8248-38aaae095445][CLUSTER-1 packages installation]]

*** CLUSTER-1 LXC initial setup

Below re the tasks as well as some tips about how to provision a CLUSTER-1 using
LXC
...
2. *Create a directory for your container*:
   #+begin_src shell :tangle no
   sudo mkdir -p /var/lib/lxc/CLUSTER-1
   #+end_src

3. *Create the container*:
   #+begin_src shell :tangle no
     sudo lxc-create --name CLUSTER-1 --template download -- --dist debian --release bookworm --arch amd64
   #+end_src

4. *Start the container*:
   #+begin_src shell :tangle no
   sudo lxc-start -n CLUSTER-1
   #+end_src

5. *Access the container’s shell*:
   #+begin_src shell :tangle no
   sudo lxc-attach -n CLUSTER-1
   #+end_src
...
1. *Stop the container* (if it is running):
   #+begin_src shell :tangle no
   sudo lxc-stop -n CLUSTER-1
   #+end_src

2. *Delete the container*:
   #+begin_src shell :tangle no
   sudo lxc-destroy -n CLUSTER-1
   #+end_src

After these commands, the =CLUSTER-1= LXC container will be removed from your
system.
...
Below there is an Ansible playbook that sets up the CLUSTER-1 container (lxc) on the
host *tsc-host-1*, performing all the tasks you've outlined:

#+begin_src yaml :tangle ansible/tasks/create-lxc-CLUSTER-1.yml

---
- name: Set up LXC container for a CLUSTER-1
  hosts: pindaro # here should be tsc-host-1 instead
  become: yes
  vars_files:
    - vars.yml
  #vars:
  #  DEST: CLUSTER-1  # remove this line if "--extra-vars "DEST=CLUSTER-1" is passed when calling ansible-playbook
...
3. Ensure you create a directory /ssh-keys/ with with the host-keys and the
   shared-keys to avoid ssh problems when container is regenerated
    #+begin_src sh :tangle no
      # this file is ansible.cfg in the root of the project
      sudo mkdir -p ssh-keys/CLUSTER-1
      sudo cp /etc/ssh/ssh_host* ssh-keys/CLUSTER-1
...
4. =Adjust your inventory file to include tsc-host-1.=
   ...
   #+begin_src conf :tangle no
     ...
     [lxc_guests]
     CLUSTER-1 ansible_hosts=10.0.3.40
     CLUSTER-1 ansible_user=concesion
     CLUSTER-1 ansible_hosts=10.0.3.11
     CLUSTER-1 ansible_user=concesion
     CLUSTER-1-2 ansible_hosts=10.0.3.12
     CLUSTER-1-2 ansible_user=concesion
     #+end_src
  
5. Run the playbook with:
   #+begin_src shell :tangle no
     cd ansible
     ansible-playbook -i inventory.ini tasks/create-lxc-CLUSTER-1.yml --extra-vars "DEST=CLUSTER-1"
   #+end_src
 
*** CLUSTER-1 packages installation.
...
#+begin_src yaml :tangle ansible/tasks/install-packages-CLUSTER-1.yml
---

- name: Set up CLUSTER-1 packages
...
1. Run the playbook with:
   #+begin_src shell :tangle no
     cd ansible 
     ansible-playbook -i inventory.ini tasks/install-packages-CLUSTER-1.yml -l CLUSTER-1
   #+end_src
