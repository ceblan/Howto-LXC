...
We split the BR-1 set up in three stages, each one with its own ansible
playbook:

1. [[id:214611e0-4085-451e-bfb1-fba2f6a2152e][BR-1 lxc playbook]]
2. [[id:6a9266c3-768a-4d72-8248-38aaae095445][BR-1 packages installation]]

*** BR-1 LXC initial setup

Below re the tasks as well as some tips about how to provision a BR-1 using
LXC
...
2. *Create a directory for your container*:
   #+begin_src shell :tangle no
   sudo mkdir -p /var/lib/lxc/BR-1
   #+end_src

3. *Create the container*:
   #+begin_src shell :tangle no
     sudo lxc-create --name BR-1 --template download -- --dist debian --release bookworm --arch amd64
   #+end_src

4. *Start the container*:
   #+begin_src shell :tangle no
   sudo lxc-start -n BR-1
   #+end_src

5. *Access the container’s shell*:
   #+begin_src shell :tangle no
   sudo lxc-attach -n BR-1
   #+end_src
...
1. *Stop the container* (if it is running):
   #+begin_src shell :tangle no
   sudo lxc-stop -n BR-1
   #+end_src

2. *Delete the container*:
   #+begin_src shell :tangle no
   sudo lxc-destroy -n BR-1
   #+end_src

After these commands, the =BR-1= LXC container will be removed from your
system.
...
Below there is an Ansible playbook that sets up the BR-1 container (lxc) on the
host *tsc-host-1*, performing all the tasks you've outlined:

#+begin_src yaml :tangle ansible/tasks/create-lxc-BR-1.yml

---
- name: Set up LXC container for a BR-1
  hosts: all # here should be tsc-host-1 instead
  become: yes
  vars_files:
    - vars.yml
  #vars:
  #  DEST: BR-1  # remove this line if "--extra-vars "DEST=BR-1" is passed when calling ansible-playbook
...
3. Ensure you create a directory /ssh-keys/ with with the host-keys and the
   shared-keys to avoid ssh problems when container is regenerated
    #+begin_src sh :tangle no
      # this file is ansible.cfg in the root of the project
      sudo mkdir -p ssh-keys/BR-1
      sudo cp /etc/ssh/ssh_host* ssh-keys/BR-1
...
4. =Adjust your inventory file to include tsc-host-1.=
   ...
   #+begin_src conf :tangle no
     ...
     [lxc_guests]
     BR-1 ansible_hosts=10.0.3.40
     BR-1 ansible_user=concesion
     BR-1 ansible_hosts=10.0.3.11
     BR-1 ansible_user=concesion
     BR-1-2 ansible_hosts=10.0.3.12
     BR-1-2 ansible_user=concesion
     #+end_src
  
5. Run the playbook with:
   #+begin_src shell :tangle no
     cd ansible
     ansible-playbook -i inventory.ini tasks/create-lxc-BR-1.yml --extra-vars "DEST=BR-1"
   #+end_src
 
*** BR-1 packages installation.
...
#+begin_src yaml :tangle ansible/tasks/install-packages-BR-1.yml
---
- name: Set up BR-1 packages
  hosts: BR-1 # here should be tsc-host-1 instead
  become_method: sudo
  become: true
  vars_files:
    - vars.yml
...
1. Run the playbook with:
   #+begin_src shell :tangle no
     cd ansible 
     ansible-playbook -i inventory.ini tasks/install-packages-BR-1.yml -l BR-1
   #+end_src
